...
\begin{table}[htp]
  ...
  \caption{FEMB-WIB connectivity for one column of APAs.  The mapping
    ...
    The pair of APAs exhibit $180^\circ$ \textbf{rotational symmetry}
    so that the low numbered connectors (1 and 2) in every crate
    service conductors attached on the face toward the central drift
    volume and the high numbered (3 and 4) connectors service
    conductors attached to the face toward the cryostat wall.  Each
    column of APAs exhibit simple translational symmetry with their
    neighbors.}
